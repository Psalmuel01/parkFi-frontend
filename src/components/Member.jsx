...
const Member = () => {
    ...
    const verify = async (exchange) => {
      ...
      const schemaId = exchange === "binance" ? ids[0] : (exchange === "bybit" ? ids[1] : (exchange === "okx" ? ids[2] : undefined));
        ...
        <div className='my-10 p-10 text-lg border-[#ccc] border rounded-lg bg-[#f5f5f5] flex flex-wrap justify-around'>
            <div>
                ...
                <div className='flex items-center justify-around'>
                    <div className='border p-4 cursor-pointer hover:bg-white' onClick={() => verify("okx")}>OKX KYC</div>
                    <div className='border p-4 cursor-pointer hover:bg-white' onClick={() => verify("bybit")}>Bybit KYC</div>
                    <div className='border p-4 cursor-pointer hover:bg-white' onClick={() => verify("binance")}>Binance KYC</div>
                </div>
            </div>
            <div className='w-full text-center mt-5 flex flex-col'>
                ...
                <small>Can only mint after KYC is verified. You will receive a membership NFT</small>
            </div>
        </div>
    </div>
  )
